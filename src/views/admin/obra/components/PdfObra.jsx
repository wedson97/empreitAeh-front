...
const PdfObra = ({ setPdfVisible, row }) => {
    ...
    return (
        <div ref={contentRef} style={styles.container}>
            ...
            <h2 style={styles.subtitle}>Detalhes da Obra</h2>
            <table style={styles.table}>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{row.orcamento.dono_obra.nome} (CPF: {row.orcamento.dono_obra.cpf}, Email: {row.orcamento.dono_obra.email})</td>
                    </tr>
                    <tr>
                        <td style={styles.label}><strong>Empreiteiro:</strong></td>
                        <td>{row.orcamento.empreiteiro.nome} (CPF: {row.orcamento.empreiteiro.cpf}, Email: {row.orcamento.empreiteiro.email})</td>
                    </tr>
                    <tr>
                        <td style={styles.label}><strong>Descrição:</strong></td>
                        <td>{row.orcamento.descricao}</td>
                    </tr>
                    <tr>
                        <td style={styles.label}><strong>Data de Criação:</strong></td>
                        <td>{new Date(row.orcamento.data_criacao).toLocaleDateString('pt-BR')}</td>
                    </tr>
                    <tr>
                        <td style={styles.label}><strong>Data de Aprovação:</strong></td>
                        <td>{new Date(row.orcamento.data_aprovacao).toLocaleDateString('pt-BR')}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>{row.orcamento.qtd_metros}</td>
                    </tr>
                    <tr>
                        <td style={styles.label}><strong>Valor por Metro:</strong></td>
                        <td>R$ {row.orcamento.valor_metro}</td>
                    </tr>
                    <tr>
                        <td style={styles.label}><strong>Endereço:</strong></td>
                        <td>{row.endereco.rua}, {row.endereco.numero} - {row.endereco.bairro}, {row.endereco.cidade} - CEP: {row.endereco.cep}</td>
                    </tr>
                </tbody>
            </table>
        </div>
